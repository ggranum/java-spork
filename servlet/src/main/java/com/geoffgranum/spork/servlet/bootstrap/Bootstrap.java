...
import com.geoffgranum.spork.servlet.SporkServletContextListener;
import com.geoffgranum.spork.servlet.exception.InvalidCommandLineException;
...
public final class Bootstrap {
  ...
  private ImmutableSet.Builder<Module> initModules() {
    ...
    SporkBootstrapModule sporkBootstrapModule = new SporkBootstrapModule(this.envSources.selectedEnv, this, applicationVersion, new InitializationChain());

    ImmutableSet.Builder<Module> modules = ImmutableSet.<Module>builder().add(sporkBootstrapModule)
                                               .add(servletModule)
                                               .addAll(this.modules);
  ...
  public <T extends SporkApplication> T start(Class<T> applicationClass) throws Exception {
    T app = this.injector().getInstance(applicationClass);
    app.start();
    return app;
  }

  public SporkServletContextListener createContextListener(BootstrapConfiguration baseConfig) {
    if(injector == null) {
      throw new InitializationException("Application must be initialized using #start().");
    }
    return new SporkServletContextListener(baseConfig.httpPort(), baseConfig.httpsPort(), injector);
  }
